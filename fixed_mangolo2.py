...
class MangoloMangoFileReader:
    ...
    @classmethod
    def loads(cls, data, file_path=None):
        ...
        while i < len(clean_lines):
            ...
            if '=' in line:
                ...
                if value_part == "":
                    ...
                    # Array/list
                    if value_part.endswith(']') and cls._is_balanced_array(value_part):
                        # Simple array on a single line
                        try:
                            array_value = cls._parse_array(value_part)
                        ...
                        while i < len(clean_lines):
                            ...
                            if ']' in next_line and cls._is_balanced_array(array_text):
                                break
                            i += 1
                        if i >= len(clean_lines) or not cls._is_balanced_array(array_text):
                            raise ParseError("Unterminated array", line_num, len(line), file_path)
                        try:
                            array_value = cls._parse_array(array_text)
                            output[keychain][key] = array_value
                        except Exception as e:
                            raise ParseError(f"Invalid array syntax: {e}", line_num, line.find(value_part) + 1, file_path)
                        i += 1
                    ...
                    value = identify_data_type(value_part)
    ...
    @classmethod
    def _parse_array(cls, array_text):
        ...
        for item in items:
            if not item:  # Skip empty items
                continue
            result.append(identify_data_type(item))

        return result

    @classmethod
    def _is_balanced_array(cls, text):
        ...
        for char in text:
            ...
            if char in ('"', "'") and (not in_quotes or quote_char == char):
                if in_quotes and quote_char == char and not escaped:
                    in_quotes = False
                    quote_char = None
                else:
                    in_quotes = True
...
class MangoloSliceFileReader:
    ...
    def loads(cls, data, file_path=None):
        ...
        for line in data.splitlines():
            ...
            if line.startswith('[') and line.endswith(']'):
                ...
                if current_section:
                    output[current_section] = cls._parse_section_content(
    ...
    @classmethod
    def _parse_section_content(cls, section_name, content, file_path):
        """Parse the content of a section based on its type."""
        if section_name == "object":
            return cls._parse_object_section(content, file_path)
        elif section_name == "indexes":
            return cls._parse_indexes_section(content, file_path)
        elif section_name == "permissions":
            return cls._parse_permissions_section(content, file_path)
        elif section_name == "validations":
            return cls._parse_validations_section(content, file_path)
        else:
            return cls._parse_generic_section(content, file_path)

    @classmethod
    def _parse_object_section(cls, content, file_path):
    ...
    @classmethod
    def _parse_indexes_section(cls, content, file_path):
    ...
    @classmethod
    def _parse_permissions_section(cls, content, file_path):
    ...
    @classmethod
    def _parse_validations_section(cls, content, file_path):
    ...
    @classmethod
    def _parse_generic_section(cls, content, file_path):
        ...
        for line, line_num in content:
            if ":" in line:
                ...
                result[key] = identify_data_type(value)
            else:
